...
class ForceLinkCommand extends Command {
  ...
  async run(message, argsString) {
    ...
    const discordUser = args[0].trim();
    const epicIGN = args[1].trim();

    if (!await doesUserExist) {
      await message.reply(`The epic ign ${epicIGN} does not exist in fortnite tracker's database`);
      return;
    }
    ...
    if (!serverMember) {
      await message.reply(`The discord account ${discordUser} could not be found within this server`);
      return;
    }

    try {
      await linkServerMemberToEpicIGN({ serverId, userDiscordId: serverMember.id, epicIGN });
      await message.reply(
        `Successfully linked your ign (${epicIGN}) to your discord account (${discordUser}, id: ${serverMember.id}).`
      );
    } catch (e) {
      console.log(e);
      await message.reply(
